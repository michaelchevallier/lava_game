...
export class CoinGenerator extends Phaser.GameObjects.Container {
  constructor(scene, x, y, opts = {}) {
    ...
    const useCoinSprite = scene.textures.exists("kenney_tile_tower_green") && scene.textures.exists("kenney_tile_base");

    if (useCoinSprite) {
      const baseSprite = scene.add.image(0, 14, "kenney_tile_base").setTint(0x3a6a1a).setScale(0.85);
      const turretSprite = scene.add.image(0, -8, "kenney_tile_tower_green").setTint(0xffd23f).setScale(0.85);
      turretSprite.setRotation(Math.PI / 2);
      const coinIcon = scene.add.image(0, -8, "kenney_tile_coin").setScale(0.5);
      this.petalGroup = scene.add.container(0, -8);
      this.petalGroup.add(coinIcon);
      this.glow = scene.add.circle(0, -8, 26, 0xffd23f, 0).setStrokeStyle(2, 0xffe066).setAlpha(0.18);
      this.add([shadow, baseSprite, turretSprite, this.glow, this.petalGroup]);
      this.heart = coinIcon;
      scene.tweens.add({ targets: this.petalGroup, angle: 360, duration: 12000, repeat: -1, ease: "Linear" });
      scene.tweens.add({ targets: coinIcon, scale: { from: 0.5, to: 0.56 }, duration: 900, yoyo: true, repeat: -1, ease: "Sine.inOut" });
      ...
      scene.tweens.add({ targets: heart, scale: { from: 1, to: 1.08 }, duration: 1200, yoyo: true, repeat: -1, ease: "Sine.inOut" });
      scene.tweens.add({ targets: this.glow, scale: { from: 1, to: 1.3 }, alpha: { from: 0.18, to: 0.05 }, duration: 1500, yoyo: true, repeat: -1, ease: "Sine.inOut" });
    }
