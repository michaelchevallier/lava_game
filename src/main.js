...
k.scene("game", () => {
  ...
  k.loop(0.5, () => { checkConstellationTrigger(); });
  ...
  function spawnVisitor() {
    ...
    v.onUpdate(() => {
      // Constellation Spectrale : override movement quand actif
      if (v._constActive) {
        const age = k.time() - v._constStart;
        if (v.vel) v.vel = k.vec2(0, 0);
        if (age < 1) {
          // Phase 1 : aura pulsante — mouvement normal arrêté, particules violettes
  ...
  function checkConstellationTrigger() {
    ...
    // Annulation si portail détruit pendant animation
    ...
    k.wait(3.5, () => {
      ...
      // Flash plein écran violet
      const flashRect = k.add([
        k.rect(WIDTH, HEIGHT),
        k.pos(0, 0),
